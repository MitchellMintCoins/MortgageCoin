...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("MTG")
    updateDisplayUnit();
}
...
{
    QString ret = QString("mortgagecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MTG unit input in lnReqAmount, we generate the URI with MTG as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MTG, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
